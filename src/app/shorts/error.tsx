...
export default function ShortsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[ShortsError]', error)
  }, [error])
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-gradient-to-br from-blue-50/60 via-white to-amber-50/50 p-4">
      <div className="glass-strong max-w-md rounded-3xl p-8 text-center shadow-xl">
        ...
        <h2 className="text-xl font-black text-slate-900">Unable to load Shorts</h2>
        <p className="mt-2 text-xs text-slate-500">
          {error?.message || 'A temporary error occurred while loading video shorts.'}
        </p>
