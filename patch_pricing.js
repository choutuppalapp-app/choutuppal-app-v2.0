...
code = code.replace(
  /className=\{cn\([\s\S]*?'mb-4 grid h-12 w-12 place-items-center rounded-2xl text-white shadow-md',[\s\S]*?p\.grad,[\s\S]*?\)\}/,
...
// Replace CTA Button
code = code.replace(
  /className=\{cn\([\s\S]*?'mt-5 w-full gap-1\.5 font-bold text-xs shadow-md',[\s\S]*?p\.highlight[\s\S]*?\? 'gradient-brand text-white'[\s\S]*?: 'bg-blue-600 text-white hover:bg-blue-700',[\s\S]*?\)\}/,
